...
var _campfire = require('campfire').Campfire;
...
_prg.version(_pjson.version).option('-u, --user <username>', 'login user name').option('-p, --pass <password>', 'login password').option('-r, --repo <repository>', 'repository for issues').option('-o, --org <organization>', 'organization').option('-t, --token <token>', 'OAuth token to use.  Password is ignored.').option('-c, --comment <comment>', 'Text to append to comment if one is created, or body of issue if creating').option('-a, --assignee <assignee>', 'assignee or "all" for all users, default is login user').option('-x, --debugjson', 'Dumps raw JSON for issues if availabled');

_prg.command('status').description('Display what each team member is currently doing').option('-w, --work', 'Displays approx. work time spent').action(function (cmd)
{
	trapaction(function ()
	{
		report(cmd.work);
...
_prg.command('doing').description('Lists in progress issues').action(trapaction(function ()
{
	list(LBL_DOING);
}));

_prg.command('backlog').description('Lists backlog issues').action(trapaction(function ()
{
	list(LBL_BACKLOG);
}));

_prg.command('ready').description('Lists ready issues').action(trapaction(function ()
{
	list(LBL_READY);
}));

_prg.command('list').description('Lists all issues').action(trapaction(function ()
{
	list()
}));

_prg.command('info #').description('Shows information on issue.  If no issue specified then shows information on issue you are currently doing.').action(trapaction(info));

_prg.command('take #').description('Take ownership of issue').action(trapaction(function (num)
{
	take(parseInt(num), false);
}));

_prg.command('create [title]').description('Creates a new issue, will be unassigned and in the backlog state').action(trapaction(function (title)
{
	create(title);
}));

_prg.command('action #').description('Take ownership of issue and put into DOING state (other assigned issues in DOING state will be switched to READY)').action(trapaction(function (num)
{
	take(parseInt(num), true);
}));

_prg.command('team').description('Lists the team members').action(trapaction(team));

_prg.command('tags [env]').description('Lists the tags').action(trapaction(function (env)
{
	tags(env);
}));
...
function loadDefaults()
{
	...
	if (_prg.campfire)
	{
		var instance = new _campfire(
		{
			ssl: true,
			token: _prg.campfire['api-token'],
			account: _prg.campfire['account-name']
		});

		debugger;
		instance.join(_prg.campfire['room-number'], function (error, room)
		{
			if (error)
			{
				console.log(error)
...
function issueState(issue)
{
	...
	if (issue.labels)
	{
		...
		{
			var lbl = _.find(LABELS_STATE, function (l)
			{
				return e.name == l;
			});
			if (lbl)
...
function issuePri(issue)
{
	...
	if (issue.labels)
	{
		_.find(issue.labels, function (e)
		{
			var lbl = _.find(LABELS_PRI, function (l)
			{
				return e.name == l;
			});
			if (lbl)
...
function commentActionee(c)
{
	if (!c || !c.body)
		return null;

	if (c.body.startsWith('[gitban] actioned onto'))
		return c.body.split(' ')[3];
	else if (c.body.startsWith('[gitban] actioned'))
		return c.user.login;
	else return null;
}
	...
	getIssues(null, LBL_DOING, function (res)
	{
		...
		if (fWork)
		{
			...
			function doit()
			{
				...
				if (issue.comments > 0)
				{
					_api.issues.getComments(
					{
						user: _prg.org,
						repo: _prg.repo,
						number: issue.number,
						per_page: 100
					}, trapapi(function (comments)
					{
						var userwork = {};
						var usercur;
						var tookcur;
						for (var i = 0; i < comments.length; i++)
						{
							...
							var actionee = commentActionee(c);
							if (actionee)
							{
								if (actionee != usercur)
								{
...
function tags(environment)
{
	if (!environment)
		throw new Error('Must specify an environment');
	
	msg('Finding tags for ' + environment);

	getRefs('tags', function (a)
	{
		var list = [];
		
		a.forEach(function(e)
		{
			var name = e.ref.split('/')[2];
			var sha = e.object.sha;
			var a = name.split('_');
			var env = a[0];
			var num = parseInt(a[1]);
		
			if (env == environment)
				list.push({num:num,sha:sha});
		});
		...
		function doit()
		{
			...
			{
				for(i=0;i<count;i++)
					console.log('%s %s', list[i].tag, list[i].dt);
				return;
			}
			
			var idx = c++;
			
			var sha = list[idx].sha;
			...
			_api.gitdata.getTag(msg, trapapi(function(t)
			{
				console.log('.');
				list[idx].tag = t.tag;
				list[idx].dt = new Date(t.tagger.date);
...
function team(cmd)
{
	...
	_api.orgs.getMembers(
	{
		org: _prg.org,
		per_page: 100
	}, trapapi(function (members)
	{
		newl();
...
function info(n)
{
	...
	function finish()
	{
		_api.issues.getRepoIssue(
		{
			user: _prg.org,
			repo: _prg.repo,
			number: num
		}, trapapi(function (issue)
		{
...
function editIssue(issue, assignee, state, comment, cb)
{
	...
	function doEdit()
	{
		...
		else msg.labels = issue.labels.map(function (e)
		{
			return e.name;
		});
...
function take(num, fAction)
{
	...
	function finish()
	{
		var txt = (assignee == _prg.user) ? (fAction ? 'actioned' : 'took') : ((fAction ? 'actioned onto ' : 'gave to ') + assignee);

		editIssue(num, assignee, fAction ? LBL_DOING : LBL_READY, txt, function (issue)
		{
...
function getRefs(type, cb)
{
	...
	// hmm.. bug: no paging seems to be supported
	_api.gitdata.getAllReferences(msg, trapapi(cb));
	...
	function doit()
	{
		...
		_api.gitdata.getAllReferences(msg, trapapi(function (res)
		{
			c += res.length;
			if (res.length == 0)
			{
...
function list(label)
{
	...
	if (isAssigneeAll(assignee))
	{
		...
		else msg('Finding all issues');
	}
	else
	{
		if (state)
			msg('Finding issues for %s in the %s state', assignee, state);
		else msg('Finding all issues assigned to %s', assignee);
...
function dump(lst, fmt)
{
	lst.sort(function (a, b)
	{
		if (a._pri == b._pri)
		{
			...
			else return LABELS_STATE.indexOf(a._state) - LABELS_STATE.indexOf(b._state);
		}
		else
		{
